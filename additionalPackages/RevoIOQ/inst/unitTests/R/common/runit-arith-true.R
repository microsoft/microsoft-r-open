...
"arith.true.stress" <- function()
{
	...
	## Need to omit the following test in R 2.14.0 and later
	## because is.nan can no longer take a list argument
	## (BTW, neither can is.finite nor is.infinite)
...
"test.arith.true.stress" <- function()
{
    res <- try(arith.true.stress())
    checkTrue(!is(res, "try-error"), msg="arith.true stress test failed")    
}
