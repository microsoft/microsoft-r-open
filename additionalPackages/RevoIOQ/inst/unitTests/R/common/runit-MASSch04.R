...
session <- RevoIOQ:::saveRUnitSession(packages=c("MASS","lattice"), datasets=c("swiss"))

"MASSch04.stress" <- function()
{
...
"test.MASSch04.stress" <- function()
{
    res <- try(MASSch04.stress())
    checkTrue(!is(res, "try-error"), msg="MASSch04 stress test failed")    
}
