...
# apparently pf(df2=Inf) worked in 2.0.1 (undocumented) but df did not.
    ...
    ## Added RUNit's checkException
    checkException(stopifnot(names(tt) == 0:6, sum(tt) == 100, sum(t2) == 100)) ## no NaN there    
...
"test.d.p.q.r.tests.stress" <- function()
{
    res <- try(d.p.q.r.tests.stress())
    checkTrue(!is(res, "try-error"), msg="d.p.q.r.tests stress test failed")    
}
