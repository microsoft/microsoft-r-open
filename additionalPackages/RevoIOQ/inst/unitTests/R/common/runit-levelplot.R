...
session <- RevoIOQ:::saveRUnitSession(packages=c("lattice"), datasets=c("volcano"))

"levelplot.stress" <- function()
{
...
"test.levelplot.stress" <- function()
{
    res <- try(levelplot.stress())
    checkTrue(!is(res, "try-error"), msg="levelplot stress test failed")    
}
