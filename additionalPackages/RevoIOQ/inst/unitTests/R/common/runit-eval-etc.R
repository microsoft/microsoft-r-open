...
"test.eval.etc.stress" <- function()
{
    res <- try(eval.etc.stress())
    checkTrue(!is(res, "try-error"), msg="eval.etc stress test failed")    
}

"testzzz.restore.session" <- function()
{
    checkTrue(RevoIOQ:::restoreRUnitSession(session), msg="Session restoration failed")
    res <- try(detach(list(x=1)))
    checkTrue(!is(res, "try-error"), msg="Detaching list(x=1) failed")    
}
