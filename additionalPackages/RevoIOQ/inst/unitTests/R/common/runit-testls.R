...
session <- RevoIOQ:::saveRUnitSession(packages=c("grid"))

"testls.stress" <- function()
{
...
"test.testls.stress" <- function()
{
    res <- try(testls.stress())
    checkTrue(!is(res, "try-error"), msg="testls stress test failed")    
}
