...
"test.R.intro.stress" <- function()
{
    res <- try(R.intro.stress())
    checkTrue(!is(res, "try-error"), msg="R.intro stress test failed")    
}

"testzzz.restore.session" <- function()
{
    checkTrue(RevoIOQ:::restoreRUnitSession(session), msg="Session restoration failed")
    res <- try(detach(faithful))
    checkTrue(!is(res, "try-error"), msg="Detaching faithful failed")    
}
