...
session <- RevoIOQ:::saveRUnitSession(packages=c("rpart"))

"usersplits.stress" <- function()
{
...
"test.usersplits.stress" <- function()
{
    res <- try(usersplits.stress())
    checkTrue(!is(res, "try-error"), msg="usersplits stress test failed")    
}
