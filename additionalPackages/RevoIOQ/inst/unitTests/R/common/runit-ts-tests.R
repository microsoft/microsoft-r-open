...
session <- RevoIOQ:::saveRUnitSession(packages=c("stats"))

"ts.tests.stress" <- function()
{
...
"test.ts.tests.stress" <- function()
{
    res <- try(ts.tests.stress())
    checkTrue(!is(res, "try-error"), msg="ts.tests stress test failed")    
}
