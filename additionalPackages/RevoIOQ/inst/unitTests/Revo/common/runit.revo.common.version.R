...
if (!identical(system.file(package="RevoScaleR") , "")) {
    ...
    "test.revor.RevoScaleR.version" <- function() checkTrue("checkPackageVersion"(pkg = "RevoScaleR", ver = revover))
...
if (!identical(system.file(package="RevoMods"), ""))
{
    ...
    "test.revor.RevoUtils.version" <- function() checkTrue("checkPackageVersion"(pkg = "RevoUtils", ver = "10.0.1"))
    "test.revor.RevoMods.version" <- function() checkTrue("checkPackageVersion"(pkg = "RevoMods", ver = "10.0.0"))
} else
{
	...
	{
		"test.revor.RevoUtils.version" <- function() checkTrue("checkPackageVersion"(pkg = "RevoUtils", ver = "10.0.0"))
	}
