...
# test url connections on http
    ...
    checkException(zz <- url("http://foo.bar", "r"))
...
"test.internet.stress" <- function()
{
    res <- try(internet.stress())
    checkTrue(!is(res, "try-error"), msg="internet stress test failed")    
}
